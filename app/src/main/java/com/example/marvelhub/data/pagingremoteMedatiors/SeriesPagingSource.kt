...
class SeriesPagingSource(private val remoteDataSource: RemoteDataSource, private val id:Int):
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, EventData> {
        ...
        return try {
            val response = remoteDataSource.getCharacterSeries(id,position+1)
            val repos = response!!.data.seriesDtos.map {
                Mappers.fromSeriesDtoToEventData(it)
            }
