...
class ComicsPagingSource(private val remoteDataSource: RemoteDataSource,private val id:Int):
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, EventData> {
        ...
        return try {
            val response = remoteDataSource.getCharacterComics(id,position+1)
            val repos = response!!.data.comicsDtos.map {
                Mappers.fromComicsDtoToEventData(comicsDto = it)
            }
